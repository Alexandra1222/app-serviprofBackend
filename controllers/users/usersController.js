...
const userRegisterController = expressAsyncHandler(async (req, res) => {
  ...
  //Check if user Exist
    ...
    if (userExists) throw new Error("Este usuario ya existe ");
    try {
      //register user
...
const forgetPasswordTokenController = expressAsyncHandler(async(req,res)=>{
  ...
  try {
    ...
    //build your message
    const resetURL = `Si se le solicitó que verificara su cuenta, verifíquela ahora dentro de 10 minutos; de lo contrario, ignore este mensaje<a href="https://appserviprof.netlify.app/resetpassword/${token}">Click para Resetear Contraseña</a>`;
    const msg = {
      to: email,
      from: "alexandrasalgan@gmail.com",
